namespace AsyncNetworking.Server
{
    using AsyncNetworking; // object serializer stored here
    using AsyncNetworking.Packets; // packets stuff stored here
    ...
    public class Server
    {
        ...
        public async Task SendToAll(object packet)
        {
            string serialized = ObjectSerializer.Serialize(packet);

            byte[] data = Encoding.ASCII.GetBytes(serialized);

            foreach (var client in clients)
            {
                //DisconnectClient(client.Key, "test");
                if (client.Value.Connected)
                    await client.Value.GetStream().WriteAsync(data, 0, data.Length);
            }
        }
    }
}